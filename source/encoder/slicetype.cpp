...
void CostEstimateGroup::estimateCUCost(LookaheadTLD& tld, int cuX, int cuY, int p0, int p1, int b, bool bDoSearch[2], bool lastRow, int slice)
{
    ...
    if (bBidir || bDoSearch[0] || bDoSearch[1])
		tld.me.setSourcePU(fenc->lowresPlane[0], fenc->lumaStride, pelOffset, cuSize, cuSize, X265_HEX_SEARCH, 1, m_lookahead.m_param->bFastSubPixMe, m_lookahead.m_param->bFastSliceType);
...
#define MVC(mv) mvc[numc++] = mv;
		...
		if (!m_lookahead.m_param->bFastSliceType)
		{
			if (!lastRow)
			{
...
#undef MVC
		...
		if (!numc)
			mvp = 0;
		else if (numc == 1 && m_lookahead.m_param->bFastSliceType)
			mvp = mvc[0];
        else
        {
