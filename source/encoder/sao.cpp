...
uint32_t SAO::saoFastModeDecide(int rowBaseAddr, int idxX, int addr)
{
	uint32_t saoMask = 0xff;
	Slice* slice = m_frame->m_encData->m_slice;
	CUData *ctu  =  m_frame->m_encData->getPicCTU(addr);
	bool isSkip = ctu->isSkipped(0);
	int cuSize  = ctu->m_log2CUSize[0];
...
void SAO::rdoSaoUnitCu(SAOParam* saoParam, int rowBaseAddr, int idxX, int addr)
{
	...
	if (m_param->bFastSao)
		m_saoTypeMask = saoFastModeDecide(rowBaseAddr, idxX, addr);
	else
		m_saoTypeMask = 0xff;

	if ((m_saoTypeMask & 0xff)== 0)
		return;
