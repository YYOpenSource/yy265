...
void MotionEstimate::setSourcePU(pixel *fencY, intptr_t stride, intptr_t offset, int pwidth, int pheight, const int method, const int refine, const int bFastSubPixMe,const int bfastLowers)
{
    ...
    searchMethod = method;
    subpelRefine = refine;
	fastSubPixMe = bFastSubPixMe;
	fastLowers = bfastLowers;
...
void MotionEstimate::setSourcePU(const Yuv& srcFencYuv, int _ctuAddr, int cuPartIdx, int puPartIdx, int pwidth, int pheight, const int method, const int refine, const int bFastSubPixMe, const int bfastLowers, bool bChroma)
{
    partEnum = partitionFromSizes(pwidth, pheight);
    X265_CHECK(LUMA_4x4 != partEnum, "4x4 inter partition detected!\n");
    sad = primitives.pu[partEnum].sad;
...
int64_t meQpelStat[2] = { 0 };
void printf_me_stat()
{
	printf("hpel skip %lld, not skip %lld,skip rate %f %\n", meQpelStat[1], meQpelStat[0],
		(double)meQpelStat[1] / (double)(meQpelStat[1] + meQpelStat[0])*100);
}
...
#undef MIN_MV
	...
	int th = 0;
	int th1 = 0;
	if (fastLowers && ref->isLowres)
		th = (blockwidth / 8) * 200;

	if (fastSubPixMe && (!ref->isLowres))
	{
		int thArray[9] = { 0, 100, 300, 300, 1000, 1000, 1000, 1000, 5000 };
		int thArrayHigh[9] = { 0, 200, 1000, 1000, 2000, 2000, 2000, 2000, 20000 };
		th = thArray[(blockwidth / 8)];
		th1 = thArrayHigh[(blockwidth / 8)]*4;//100 - 100% skip; 6 -85% skip,35.23x; 4 - 65% skip,35.247; 3 - 48% 35.24x; 2 - skip 23%,35.255
	}
