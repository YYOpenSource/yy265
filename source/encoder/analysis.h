...
//#define YY_FAST_SKIP_DETECTION 0
...
protected:
    ...
    uint64_t* cacheCost;
	fastCuHelper intraCuSplitHelper;
	...
	void checkMerge2Nx2N_rd0_4(const CUData& parentCTU, Mode& skip, Mode& merge, const CUGeom& cuGeom, double globalSipPr=0.0);
	...
	bool recursionDepthCheck(const CUData& parentCTU, const CUGeom& cuGeom, const Mode& bestMode, double thred = 1.0);
    bool complexityCheckCU(const Mode& bestMode);

	/* yy fast skip detection: GLFS */
	bool GLFSfastSkipCheck(const CUData& parentCTU, const CUGeom& cuGeom, const Mode& bestMode, double globalSkipPr);
