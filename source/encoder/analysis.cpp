...
#define CLAC_STAT 1
#define FAST_INTRA_CU_SPLIT 0

#define FAST_INTRA_CU_DEBUG_OPEN 0

#if FAST_INTRA_CU_DEBUG_OPEN 
#define FAST_INTRA_PRINFT(x,...) printf(x,__VA_ARGS__)
#else
#define FAST_INTRA_PRINFT(x,...) 
#endif
...
Mode& Analysis::compressCTU(CUData& ctu, Frame& frame, const CUGeom& cuGeom, const Entropy& initialContext)
{
	...
	assert(m_modeDepth[0].fencYuv.m_part == 4);
    ...
    if (m_slice->m_sliceType == I_SLICE)
    {
		...
		if (m_param->bEnableIntraFastCuSplit)
		{
			pixel  *ctuEnc = m_modeDepth[0].fencYuv.m_buf[0];//pixel y
			int    stride = m_modeDepth[0].fencYuv.m_size;
			int    mpart = m_modeDepth[0].fencYuv.m_part;

			intraCuSplitHelper.loadFencCtu(ctuEnc, stride);
		}
...
void Analysis::compressIntraCU(const CUData& parentCTU, const CUGeom& cuGeom, int32_t qp)
{
	...
	INTRA_CUSPILIT split = INTRA_CU_OTHERS;
	if (m_param->bEnableIntraFastCuSplit)
	{
		if ( cuGeom.log2CUSize != MAX_LOG2_CU_SIZE )
		{
			int size = (1 << cuGeom.log2CUSize);
			split = intraCuSplitHelper.cuSplitDecideVar(size, cuGeom.absPartIdx / 4, qp);

			if (split == INTRA_CU_SPLIT)
			{
				FAST_INTRA_PRINFT("fast cu split: ret split cu! cusize %d, partIdx %d. org mightNotSplit %d\n", size, cuGeom.absPartIdx / 4, mightNotSplit);
				//如果是8x8，没有split模式，只有2nx2n模式。mightNotSplit必须为true
				if (!(cuGeom.flags & CUGeom::LEAF))
					mightNotSplit &= false;
				else
					FAST_INTRA_PRINFT("leaf cu, not split \n");
			}
			else if (split == INTRA_CU_NOT_SPLIT)
			{
				FAST_INTRA_PRINFT("fast cu split:ret not split Cu, cusize %d, partIdx %d.org mightSplit %d\n", size, cuGeom.absPartIdx / 4, mightSplit);
				//如果强制split，这里的notSplit应不能起作用
				if (!(cuGeom.flags & CUGeom::SPLIT_MANDATORY))
					mightSplit &= false;
				else
					FAST_INTRA_PRINFT("MANDATORY split\n");
			}
			else
			{
				FAST_INTRA_PRINFT("can not decede!!\n");
			}
		}
	}
    ...
    else if (cuGeom.log2CUSize != MAX_LOG2_CU_SIZE && mightNotSplit)
    {
		if (m_param->bEnableIntraFastCuSplit)
		{
			...
			//	}
			//	else
			//		FAST_INTRA_PRINFT("8x8 skip 2nx2n \n");
			//}
			//else 
			{
				md.pred[PRED_INTRA].cu.initSubCU(parentCTU, cuGeom, qp);
			...
			if (cuGeom.log2CUSize == 3 && m_slice->m_sps->quadtreeTULog2MinSize < 3)
			{
				if (split != INTRA_CU_NOT_SPLIT)
				{
					...
					FAST_INTRA_PRINFT("8x8 skip nxn \n");
...
uint32_t Analysis::compressInterCU_dist(const CUData& parentCTU, const CUGeom& cuGeom, int32_t qp)
{
    ...
    if (mightNotSplit && depth >= minDepth)
    {
        ...
        if (m_param->rdLevel <= 4)
			checkMerge2Nx2N_rd0_4(parentCTU,md.pred[PRED_SKIP], md.pred[PRED_MERGE], cuGeom);
        else
...
uint64_t fastRecursionCuSplit[4][4] = { 0 };
SplitData Analysis::compressInterCU_rd0_4(const CUData& parentCTU, const CUGeom& cuGeom, int32_t qp)
{
	...
	/* yy GLFS fast skip. TODO: to be refined for WPP multi-threading. */
	static uint64_t frameSkipCount[4] = {0};
	static uint64_t prevFrameSkipCount[4] = {0};
	if ( parentCTU.m_cuAddr==0 && cuGeom.depth==0 )
	{
		uint64_t tmp;
		/*tmp = prevFrameSkipCount[0];
		prevFrameSkipCount[0] = frameSkipCount[0];
		frameSkipCount[0] = tmp;
		tmp = prevFrameSkipCount[1];
		prevFrameSkipCount[1] = frameSkipCount[1];
		frameSkipCount[1] = tmp;
		tmp = prevFrameSkipCount[2];
		prevFrameSkipCount[2] = frameSkipCount[2];
		frameSkipCount[2] = tmp;
		tmp = prevFrameSkipCount[3];
		prevFrameSkipCount[3] = frameSkipCount[3];
		frameSkipCount[3] = tmp;*/
		/*prevFrameSkipCount[0] = frameSkipCount[0];
		prevFrameSkipCount[1] = frameSkipCount[1];
		prevFrameSkipCount[2] = frameSkipCount[2];
		prevFrameSkipCount[3] = frameSkipCount[3];*/
	...
	uint64_t totSkipCount = frameSkipCount[0] + frameSkipCount[1] + frameSkipCount[2] + frameSkipCount[3]/* + prevFrameSkipCount[0] + prevFrameSkipCount[1] + prevFrameSkipCount[2] + prevFrameSkipCount[3]*/;
	double curGlobalSkipPr = totSkipCount==0 ? 0 : ( ((double)frameSkipCount[depth]/* + (double)prevFrameSkipCount[depth]*/)/ totSkipCount);
    ...
    if (mightNotSplit && depth >= minDepth && !md.bestMode) /* TODO: Re-evaluate if analysis load/save still works */
    {
        ...
        md.pred[PRED_MERGE].cu.initSubCU(parentCTU, cuGeom, qp);
        md.pred[PRED_SKIP].cu.initSubCU(parentCTU, cuGeom, qp);
		checkMerge2Nx2N_rd0_4(parentCTU, md.pred[PRED_SKIP], md.pred[PRED_MERGE], cuGeom, curGlobalSkipPr);
        if (m_param->rdLevel)
            skipModes = m_param->bEnableEarlySkip && md.bestMode && md.bestMode->cu.isSkipped(0); // TODO: sa8d threshold per depth

		/* added by zhangyihao */
#if 0
		if ( YY_FAST_SKIP_DETECTION && m_param->bEnableEarlySkip && md.bestMode && !skipModes )
		{
			int totSkipCount = (frameSkipCount[0]+frameSkipCount[1]+frameSkipCount[2]+frameSkipCount[3]);
			if (totSkipCount>=2)
			{
				double globalSkipPr = ((double)frameSkipCount[depth]) / totSkipCount;
				skipModes = GLFSfastSkipCheck(parentCTU, cuGeom, *md.bestMode, globalSkipPr);
				//isSkipped = skipModes;
			}
		}
#endif
...
#if CLAC_STAT
		int depth = md.bestMode->cu.m_cuDepth[0];
		if (skipRecursion)	fastRecursionCuSplit[depth][0] ++;
        if (mightSplit && depth >= minDepth && !skipRecursion)
        {
            if (depth)
                skipRecursion = recursionDepthCheck(parentCTU, cuGeom, *md.bestMode);
			if (skipRecursion)		fastRecursionCuSplit[depth][1] ++;
            if (m_bHD && !skipRecursion && m_param->rdLevel == 2 && md.fencYuv.m_size != MAX_CU_SIZE)
                skipRecursion = complexityCheckCU(*md.bestMode);
			if (skipRecursion)		fastRecursionCuSplit[depth][2] ++;
        }
		if (!skipRecursion)  fastRecursionCuSplit[depth][3] ++;
#else
		if (mightSplit && depth >= minDepth && !skipRecursion)
		{
			if (depth)
	...
	/* yy GLFS fast skip, fix the global skip probability */
	int nextDepthSkipCount = 0;
	uint64_t nextDepthSkipRDcost = 0;
    ...
    if (mightSplit && !skipRecursion)
    {
        ...
        for (uint32_t subPartIdx = 0; subPartIdx < 4; subPartIdx++)
        {
            ...
            if (childGeom.flags & CUGeom::PRESENT)
            {
				...
				/* yy GLFS fast skip, fix the global skip probability */
				if (nd.bestMode->cu.isSkipped(0))
				{
					nextDepthSkipCount += 1;
					nextDepthSkipRDcost += nd.bestMode->rdCost;
				}
    ...
    if (mightSplit && !skipRecursion)
    {
		...
		/* yy GLFS fast skip, fix the global skip probability */
		if ( md.bestMode != &md.pred[PRED_SPLIT] && depth < 3 )
		{
		
			int next_depth = depth+1;
			frameSkipCount[next_depth] -= nextDepthSkipCount;
#if 0
			FrameData& curEncData = *m_frame->m_encData;
			FrameData::RCStatCU& cuStat = curEncData.m_cuStat[parentCTU.m_cuAddr];
			uint64_t temp = cuStat.avgCost[next_depth] * cuStat.count[next_depth];
			cuStat.count[next_depth] -= nextDepthSkipCount;
			cuStat.avgCost[next_depth] = cuStat.count[next_depth]!=0?(temp - nextDepthSkipRDcost) / cuStat.count[next_depth]:0;
			
			assert(cuStat.count[next_depth]>=0);
			assert(cuStat.avgCost[next_depth]>=0);
#endif
			assert(frameSkipCount[next_depth]>=0);
...
uint64_t fastRecursionSkipStat[4][5] = { 0 };//fastRecursionSkipStat[depth][skipstat]

#define H1 if (level >= X265_LOG_DEBUG) printf
void fastRecursionSkipStatLogOut()
{
	for (int depth = 0; depth < 4; depth++)
	{
		printf("depth %d ,---------------------------\n", depth);
		printf("fast skip: skip %lld,not skip %lld \n", fastRecursionSkipStat[depth][1],fastRecursionSkipStat[depth][0] );
		printf("detect by mergeSkip %lld, merge mode %lld,mSkip not match %lld \n", fastRecursionSkipStat[depth][2], fastRecursionSkipStat[depth][4],fastRecursionSkipStat[depth][3]);
		printf("hit mergeSkip cu not split %lld, m-skip&recursion cu not split %lld, m-skip&recursion&complex cu not split %lld, cu split %lld \n",
			fastRecursionCuSplit[depth][0], fastRecursionCuSplit[depth][1], fastRecursionCuSplit[depth][2], fastRecursionCuSplit[depth][3]);
	}
...
void Analysis::checkMerge2Nx2N_rd0_4(const CUData& parentCTU,Mode& skip, Mode& merge, const CUGeom& cuGeom, double globalSkipPr)
{
    uint32_t depth = cuGeom.depth;
    ModeDepth& md = m_modeDepth[depth];
    ...
    if (m_param->rdLevel)
    {
		...
		if (m_param->bEnableFastRecursionSkip){
			if (recursionDepthCheck(parentCTU, cuGeom, *bestPred, 0.9) && depth)
			{
				md.bestMode = bestPred;
				#if CLAC_STAT
				fastRecursionSkipStat[depth][1] ++;
				#endif
				goto RECURSION_SKIP;
			}
		}

		if (/*YY_FAST_SKIP_DETECTION || */m_param->bEnableGLFSfastSkip)
		{
			if (GLFSfastSkipCheck(parentCTU, cuGeom, *bestPred, globalSkipPr))
			{
				md.bestMode = bestPred;
				fastRecursionSkipStat[depth][1] ++;
				goto RECURSION_SKIP;
		...
		if (tempPred->rdCost < bestPred->rdCost)
			fastRecursionSkipStat[depth][4] ++;
		if ( md.bestMode->cu.isSkipped(0) )
			fastRecursionSkipStat[depth][2] ++;
		else
			fastRecursionSkipStat[depth][3] ++;
		#endif
    }
    else
        md.bestMode = bestPred;
...
bool Analysis::recursionDepthCheck(const CUData& parentCTU, const CUGeom& cuGeom, const Mode& bestMode,double thred)
{
    ...
    if (neighCount + cuCount)
    {
        ...
        if (curCost < (thred*avgCost) && avgCost)
            return true;
    }
...
/* yy fast skip detection, GLFS */
bool Analysis::GLFSfastSkipCheck(const CUData& parentCTU, const CUGeom& cuGeom, const Mode& bestMode, double globalSkipPr)
{
	uint32_t depth = cuGeom.depth;
    FrameData& curEncData = *m_frame->m_encData;
    FrameData::RCStatCU& cuStat = curEncData.m_cuStat[parentCTU.m_cuAddr];
	...
	{
		totCount += cuStat.count[i];
		curCount += cuStat.count[i];
	}
    ...
    if (left)
    {
		...
		for ( int i = 0; i < g_maxCUDepth; i++ )
		{
				totCount += nstat.count[i];
		}
    }
	/*uint64_t predCost = cuCount>=2 ? cuCost:(cuCost + neighCost);
	uint64_t predCount =  cuCount>=2 ? cuCount:(cuCount + neighCount);*/

	uint64_t predCost = cuCost + neighCost;
	uint64_t predCount = cuCount + neighCount;
	if (predCount==0 || totCount==0)
	{
		return false;
	}
	double thCost = ((double)predCost) / predCount;
	/*double localSkipPr = cuCount >= 2? (((double)predCount) / curCount) : (((double)predCount) / totCount);*/
	double localSkipPr = ((double)predCount) / totCount;
	double weight = 1.0;// weighting factor.
	//double globalSkipPr;

	//at least 2 neighboring skip mode CUs exsit. RDcost < thCost && local_skip_probabilty > global_skip_probablity.
	if ( predCount >= 10 && bestMode.rdCost < (weight * thCost) && localSkipPr > 1.0*globalSkipPr )
	{
		return true;
	}
