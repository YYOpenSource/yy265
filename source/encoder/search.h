...
protected:
	...
	//satd select best chroma mode from luma; result is fully encode chroma. chroma distortion is returned
	sse_t estIntraPredChromaQT_satd(Mode &intraMode, const CUGeom& cuGeom);
	...
	uint16_t  lastIntraCandidate[5];//best 3 candidate + count + best candidate 
	int sortCandidate(uint64_t candCostList[MAX_RD_INTRA_MODES], uint32_t rdModeList[MAX_RD_INTRA_MODES], int maxCandCount, uint64_t candCostListTmp[MAX_RD_INTRA_MODES], uint32_t rdModeListTmp[MAX_RD_INTRA_MODES]);
	int excludeCandForRdo(uint64_t candCostListSorted[MAX_RD_INTRA_MODES], uint32_t rdModeListSorted[MAX_RD_INTRA_MODES], int maxCandCountSorted, uint64_t candCostList[MAX_RD_INTRA_MODES], uint32_t rdModeList[MAX_RD_INTRA_MODES], uint32_t mpmModes[3]);
	bool checkIntraSkipRdo(Mode& mIntra,uint64_t candCostListSorted[MAX_RD_INTRA_MODES], uint32_t rdModeListSorted[MAX_RD_INTRA_MODES], int maxCandCountSorted);
