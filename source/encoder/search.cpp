...
bool Search::initSearch(const x265_param& param, ScalingList& scalingList)
{
	...
	memset(lastIntraCandidate, 0, sizeof(lastIntraCandidate));
...
void Search::checkIntra(Mode& intraMode, const CUGeom& cuGeom, PartSize partSize)
{
    ...
    {
		if (m_param->bFastChromaIntra)
			intraMode.chromaDistortion += estIntraPredChromaQT_satd(intraMode, cuGeom);
		else
...
int Search::sortCandidate(uint64_t candCostList[MAX_RD_INTRA_MODES], uint32_t rdModeList[MAX_RD_INTRA_MODES], int maxCandCount, uint64_t candCostListTmp[MAX_RD_INTRA_MODES], uint32_t rdModeListTmp[MAX_RD_INTRA_MODES])
{
	int maxCandCountAvaible = 0;

	for (int i = 0; i < maxCandCount; i++)
	{
		if (candCostList[i] != MAX_INT64)
		{
			candCostListTmp[maxCandCountAvaible] = candCostList[i];
			rdModeListTmp[maxCandCountAvaible] = rdModeList[i];
			maxCandCountAvaible++;
		}
	}

	//sort
	for (int i = 0; i < maxCandCountAvaible; i++)
	{
		uint64_t costTmp = MAX_INT64;
		uint32_t modeTmp = 0;
		for (int j = i + 1; j < maxCandCountAvaible; j++)
		{
			if (candCostListTmp[i] > candCostListTmp[j])
			{
				costTmp = candCostListTmp[i];
				candCostListTmp[i] = candCostListTmp[j];
				candCostListTmp[j] = costTmp;

				modeTmp = rdModeListTmp[i];
				rdModeListTmp[i] = rdModeListTmp[j];
				rdModeListTmp[j] = modeTmp;
			}
		}
	}
	return maxCandCountAvaible;
}

bool Search::checkIntraSkipRdo(Mode& mIntra, uint64_t candCostListSorted[MAX_RD_INTRA_MODES], uint32_t rdModeListSorted[MAX_RD_INTRA_MODES], int maxCandCountSorted)
{
	uint16_t *lastIntraMode = lastIntraCandidate;
	int lastCuCandCount = lastIntraMode[3];
	bool ret = false;

	if ( lastCuCandCount > 0  && lastCuCandCount <= maxCandCountSorted){
		for (int i = 0; i < lastCuCandCount; i++){
			if (lastIntraMode[i] != rdModeListSorted[i])
				goto CHECK_FAILED;
		}
		ret = true;
	}
...
CHECK_FAILED:
	for (int i = 0; i < maxCandCountSorted; i++){
		if (i < 3)
			lastIntraMode[i] = rdModeListSorted[i];
	}
	lastIntraMode[3] = maxCandCountSorted > 3 ? 3 : maxCandCountSorted;

	return ret;
}

int Search::excludeCandForRdo(uint64_t candCostListSorted[MAX_RD_INTRA_MODES], uint32_t rdModeListSorted[MAX_RD_INTRA_MODES], int maxCandCountSorted, uint64_t candCostList[MAX_RD_INTRA_MODES], uint32_t rdModeList[MAX_RD_INTRA_MODES], uint32_t mpmModes[3])
{
	int maxCandCountAvaible = maxCandCountSorted;

	//drop after mpm cand
	int lastMpMCand = maxCandCountAvaible;
	int excludeCandCount = 0;
	for (int i = 0; i < maxCandCountAvaible; i++){
		if (rdModeListSorted[i] == mpmModes[0] ||
			rdModeListSorted[i] == mpmModes[1] ||
			rdModeListSorted[i] == mpmModes[2])
		{
			lastMpMCand = i;
		}
	}

	if (lastMpMCand < maxCandCountAvaible){
		for (int i = lastMpMCand + 1; i < maxCandCountAvaible; i++){
			//至少保留3个
			if ( i > 2 ){
				candCostListSorted[i] = MAX_INT64;
				rdModeListSorted[i] = MAX_UINT;
				excludeCandCount++;
			}
		}
		maxCandCountAvaible -= excludeCandCount;
		//printf("exclude mpm last after id %d,mode %d\n", lastMpMCand,rdModeListTmp[lastMpMCand]);
		//printf("maxCandCountAvaible %d,mode %d,%d \n", maxCandCountAvaible, rdModeListTmp[0], rdModeListTmp[1]);
	}

	//reduce the neigbour too close
	int maxCandCountLast = maxCandCountAvaible;
	for (int i = (maxCandCountAvaible - 1); i > 1; i--){
		//dc and planar mode do not exclude
		if (rdModeListSorted[i] < 2)
			continue;

		for (int preMode = 0; preMode < i; preMode++){
			int diff = rdModeListSorted[preMode] - rdModeListSorted[i];

			if (rdModeListSorted[preMode] >= 2 && rdModeListSorted[i] >= 2
				&& abs(diff) <= 2){
				//exclude 
				rdModeListSorted[i] = MAX_UINT;
				candCostListSorted[i] = MAX_INT64;
	...
	//copy back
	int modeCountFinal = 0;
	for (int i = 0; i < maxCandCountSorted; i++){
		if (candCostListSorted[i] != MAX_INT64){
			candCostList[modeCountFinal] = candCostListSorted[i];
			rdModeList[modeCountFinal] = rdModeListSorted[i];
			modeCountFinal++;
		}
		else{
			candCostList[i] = MAX_INT64;
			rdModeList[i] = MAX_UINT;
		}
	}

	assert(maxCandCountLast == modeCountFinal);
	//if (modeCountFinal < maxCandCountSorted)
		//printf("intra rdoq reduce, reduce cand from %d to %d \n", maxCandCountSorted, modeCountFinal);

	return modeCountFinal;
}
sse_t Search::estIntraPredQT(Mode &intraMode, const CUGeom& cuGeom, const uint32_t depthRange[2])
{
    ...
    for (uint32_t puIdx = 0; puIdx < numPU; puIdx++, absPartIdx += qNumParts)
    {
        ...
        else
        {
			...
			//reduece candi count
			if (m_param->bFastIntra2) {
				int candCountHM[5] = { 3, 3, 8, 8, 8 };
				if (maxCandCount > candCountHM[depth])
				{
					maxCandCount = candCountHM[depth];
				}
			}

            {
                ...
                *  non-mpm modes    - all cost the same (rbits) */
				...
				//no rdo
				if (m_param->bFastIntra2 && m_param->bNoIntraRdo)
				{
					int bestMode = 0;
					uint64_t bestCost = MAX_INT64;
					//fast intra
					for (int mode = 5; mode < 35; mode+=5)
					{
						bits = (mpms & ((uint64_t)1 << mode)) ? m_entropyCoder.bitsIntraModeMPM(mpmModes, mode) : rbits;
						...
						COPY2_IF_LT(bestCost, modeCosts[mode], bestMode, mode);
					}

					for (uint32_t dist = 2; dist >= 1; dist--)
					{
						int lowmode = bestMode - dist;
						int highmode = bestMode + dist;

						bits = (mpms & ((uint64_t)1 << lowmode)) ? m_entropyCoder.bitsIntraModeMPM(mpmModes, lowmode) : rbits;
						int filter = !!(g_intraFilterFlags[lowmode] & scaleTuSize);
						primitives.cu[sizeIdx].intra_pred[lowmode](m_intraPred, scaleTuSize, intraNeighbourBuf[filter], lowmode, scaleTuSize <= 16);
						sad = sa8d(fenc, scaleStride, m_intraPred, scaleTuSize) << costShift;
						modeCosts[lowmode] = m_rdCost.calcRdSADCost(sad, bits);
						COPY2_IF_LT(bestCost, modeCosts[lowmode], bestMode, lowmode);

						bits = (mpms & ((uint64_t)1 << highmode)) ? m_entropyCoder.bitsIntraModeMPM(mpmModes, highmode) : rbits;
						filter = !!(g_intraFilterFlags[highmode] & scaleTuSize);
						primitives.cu[sizeIdx].intra_pred[highmode](m_intraPred, scaleTuSize, intraNeighbourBuf[filter], highmode, scaleTuSize <= 16);
						sad = sa8d(fenc, scaleStride, m_intraPred, scaleTuSize) << costShift;
						modeCosts[highmode] = m_rdCost.calcRdSADCost(sad, bits);
						COPY2_IF_LT(bestCost, modeCosts[highmode], bestMode, highmode);
					...
					COPY2_IF_LT(bestCost, modeCosts[DC_IDX], bestMode, DC_IDX);
					COPY2_IF_LT(bestCost, modeCosts[PLANAR_IDX], bestMode, PLANAR_IDX);
				...
				* levels and at higher depths */
				...
				if (m_param->bFastIntra2){
					uint64_t candCostListSort[MAX_RD_INTRA_MODES];
					uint32_t rdModeListSort[MAX_RD_INTRA_MODES];
					int candCountSort;
					candCountSort = sortCandidate(candCostList, rdModeList, maxCandCount, candCostListSort, rdModeListSort);
					skipRdo = checkIntraSkipRdo(intraMode, candCostListSort, rdModeListSort, candCountSort);
					maxCandCount = excludeCandForRdo(candCostListSort, rdModeListSort, candCountSort, candCostList, rdModeList, mpmModes);
				}
			}

			if (m_param->bFastIntra2 && skipRdo)
			{
				bmode = lastIntraCandidate[4];
				//printf("skip intra rdo \n");
				goto RDO_SKIP;
			}
...
int64_t bestModeStat[5] = { 0 };
void printf_chroma_intra_mode()
{
	printf("intra chroma stat %lld,%lld,%lld,%lld,%lld \n", bestModeStat[0], bestModeStat[1], bestModeStat[2], bestModeStat[3], bestModeStat[4]);
}
sse_t Search::estIntraPredChromaQT_satd(Mode &intraMode, const CUGeom& cuGeom)
{
	CUData& cu = intraMode.cu;
	Yuv& reconYuv = intraMode.reconYuv;
	...
	do
	{
		...
		//check best mode with sa8d
#if 0
		uint64_t modeCosts[37] = { 0 };
		for (uint32_t mode = minMode; mode < maxMode; mode++)
		{
			...
			for (uint32_t chromaId = TEXT_CHROMA_U; chromaId <= TEXT_CHROMA_V; chromaId++)
			{
				...
				pixel*   pred = intraMode.predYuv.getChromaAddr(chromaId, absPartIdxC);
				const pixel* fenc = intraMode.fencYuv->getChromaAddr(chromaId, absPartIdxC);
				uint32_t strideFenc = intraMode.fencYuv->m_csize;
				uint32_t stridePred = intraMode.predYuv.m_csize;

				int tuSize = 1 << log2TrSizeC;

				X265_CHECK(sizeIdx >= 0 && sizeIdx < 4, "intra block size is out of range\n");

				bit = 0;//mode costs bit
				int filter = !!(m_csp == X265_CSP_I444 && (g_intraFilterFlags[chromaPredMode] & tuSize));
				primitives.cu[sizeIdx].intra_pred[chromaPredMode](pred, stridePred, intraNeighbourBuf[filter], chromaPredMode, 0);

				sad += sa8d(fenc, strideFenc, pred, stridePred);
			}
			modeCosts[chromaPredMode] = m_rdCost.calcRdSADCost(sad, bit);
			COPY2_IF_LT(bestCost, modeCosts[chromaPredMode], bestMode, mode);

		}
#endif
		int selectMode = 4;//bestMode
		int modeStat = 0;
		bestMode = 0;
		bestDist = 0;
		bestCost = MAX_INT64;
		...
		// check chroma modes
		for (uint32_t mode = selectMode; mode <= selectMode; mode++)
		{
...
void Search::processPME(PME& pme, Search& slave)
{
    ...
    if (&slave != this)
    {
		...
		slave.m_me.setSourcePU(*pme.mode.fencYuv, pme.pu.ctuAddr, pme.pu.cuAbsPartIdx, pme.pu.puAbsPartIdx, pme.pu.width, pme.pu.height, m_param->searchMethod, m_param->subpelRefine, m_param->bFastSubPixMe, m_param->bFastSliceType, bChroma);
    }
...
void Search::predInterSearch(Mode& interMode, const CUGeom& cuGeom, bool bChromaMC, uint32_t refMasks[2])
{
    ...
    for (int puIdx = 0; puIdx < numPart; puIdx++)
    {
		...
		m_me.setSourcePU(*interMode.fencYuv, pu.ctuAddr, pu.cuAbsPartIdx, pu.puAbsPartIdx, pu.width, pu.height, m_param->searchMethod, m_param->subpelRefine, m_param->bFastSubPixMe, m_param->bFastSliceType,bChromaMC);
